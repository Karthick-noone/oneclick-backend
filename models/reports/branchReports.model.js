...
exports.getProductCountByCategory = (branch_id, userRole, callback) => {
  let query = `
    SELECT 
      category, 
      COUNT(*) AS total_products,
      SUM(CASE WHEN productStatus = 'unapproved' THEN 1 ELSE 0 END) AS unapproved_products
    FROM oneclick_product_category
    WHERE 1
  `;
  const params = [];
  ...
  if (branch_id) {
    // specific branch → only that branch
    query += " AND branch_id = ?";
    params.push(branch_id);
  } else {
    // default → all branch products (NOT NULL)
    query += " AND branch_id IS NOT NULL";
  }
...
exports.getTotalProducts = (branch_id, userRole, callback) => {
  let query = `SELECT COUNT(*) AS total_products FROM oneclick_product_category WHERE 1`;
  const params = [];
...
// exports.getStaffCounts = (branch_id, userRole, callback) => {
//   let query = `SELECT COUNT(*) AS staffCount FROM oneclick_staff WHERE 1`;
//   const params = [];

//   if (branch_id && userRole === "branch_admin") {
//     query += " AND branch_id = ?";
//     params.push(branch_id);
//   } else {
//     query += " AND branch_id IS NOT NULL";
