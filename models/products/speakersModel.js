...
exports.addProduct = (productData, callback) => {
  console.log("[Speaker][Insert] Incoming Data:", productData);
  ...
  db.query(sql, Object.values(productData), (err, result) => {
    if (err) {
      console.error("[Speaker][Insert] ERROR:", err);
      return callback({ status: 500, message: "Error adding product" });
    }

    console.log("[Speaker][Insert] SUCCESS — ID:", result.insertId);

    // --- notification logic ---
    if (productData.user_role === "branch_admin" || productData.user_role === "Staff") {

      const message = `Speakeres: "${productData.prod_name}" added by ${productData.contact_person}(${productData.user_role === "branch_admin" ? "Branch Admin" : "Staff"}).`;
...
exports.updateProductImage = (productId, imageIndex, newImage, callback) => {
    ...
    db.query(sqlSelect, [productId], (err, results) => {
      ...
      const oldImagePath = path.join("uploads", "speakers", oldImages[imageIndex]);
...
exports.deleteProductImage = (productId, imageIndex, callback) => {
    ...
    db.query(oldImageQuery, [productId], (err, results) => {
      ...
      const oldImagePath = path.join("uploads", "speakers", imageToDelete);
...
exports.uploadspeakersImages = (req, res) => {
    ...
    speakersModel.updateProductImages(productId, newImages, (err, message) => {
