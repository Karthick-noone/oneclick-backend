...
exports.addProduct = (productData, callback) => {
  console.log("[Watch][Insert] Incoming Data:", productData);
  ...
  db.query(sql, Object.values(productData), (err, result) => {
    ...
    console.log("[Watch][Insert] SUCCESS — ID:", result.insertId);

    // --- notification logic ---
    if (productData.user_role === "branch_admin" || productData.user_role === "Staff") {

      const message = `Watches: "${productData.prod_name}" added by ${productData.contact_person}(${productData.user_role === "branch_admin" ? "Branch Admin" : "Staff"}).`;
...
exports.updateProductImage = (productId, imageIndex, newImage, callback) => {
    ...
    db.query(sqlSelect, [productId], (err, results) => {
      ...
      const oldImagePath = path.join("uploads", "watch", oldImages[imageIndex]);
...
exports.deleteProductImage = (productId, imageIndex, callback) => {
    ...
    db.query(oldImageQuery, [productId], (err, results) => {
      ...
      const oldImagePath = path.join("uploads", "watch", imageToDelete);
...
exports.uploadwatchImages = (req, res) => {
    ...
    watchModel.updateProductImages(productId, newImages, (err, message) => {
