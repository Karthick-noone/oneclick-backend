...
exports.addProduct = (productData, callback) => {
  console.log("[Computer Accessories][Insert] Incoming Data:", productData);
  ...
  db.query(sql, Object.values(productData), (err, result) => {
    if (err) {
      console.error("[Computer Accessories][Insert] ERROR:", err);
      return callback({ status: 500, message: "Error adding product" });
    }

    console.log("[Computer Accessories][Insert] SUCCESS — ID:", result.insertId);

    // --- notification logic ---
    if (productData.user_role === "branch_admin" || productData.user_role === "Staff") {

      const message = `Computer Accessories: "${productData.prod_name}" added by ${productData.contact_person}(${productData.user_role === "branch_admin" ? "Branch Admin" : "Staff"}).`;
...
exports.fetchApprovedProducts = (callback) => {
  ...
  const sql = `
    ...
    WHERE 
      p.category = 'computeraccessories'
...
exports.updateProductImage = (productId, imageIndex, newImage, callback) => {
    ...
    db.query(sqlSelect, [productId], (err, results) => {
      ...
      const oldImagePath = path.join("uploads", "computeraccessories", oldImages[imageIndex]);
...
exports.deleteProductImage = (productId, imageIndex, callback) => {
    ...
    db.query(oldImageQuery, [productId], (err, results) => {
      ...
      const oldImagePath = path.join("uploads", "computeraccessories", imageToDelete);
...
exports.uploadcomputeraccessoriesImages = (req, res) => {
    ...
    computeraccessoriesModel.updateProductImages(productId, newImages, (err, message) => {
